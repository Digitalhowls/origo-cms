...
function zn_check_components( $component ) {
	...
	if ( isset( $component ) && ! empty( $component ) ) {
		...
		if ( $component == 'search_box' ) {
			return zget_option( 'head_show_search', 'general_options', false, 'yes' ) == 'yes' ? true : false;
		} elseif ( $component == 'flags' ) {
			return zget_option( 'head_show_flags', 'general_options', false, 1 ) == 1 ? true : false;
		} elseif ( $component == 'login' ) {
			return zget_option( 'head_show_login', 'general_options', false, 1 ) == 1 ? true : false;
		} elseif ( $component == 'register' ) {
			return zget_option( 'head_show_register', 'general_options', false, 2 ) == 1 ? true : false;
		} elseif ( $component == 'custom_text' ) {
			return ! empty( $topTextVal ) ? true : false;
		} elseif ( $component == 'hidden_panel' ) {
			return zget_option( 'head_show_support_pnl', 'general_options', false, 'yes' ) == 'yes' ? true : false;
		} elseif ( $component == 'social_icons' ) {
			return zget_option( 'social_icons_visibility_status', 'general_options', false, 'yes' ) == 'yes' ? true : false;
		} elseif ( $component == 'header_nav' ) {
			return has_nav_menu( 'header_navigation' ) && wp_nav_menu(
				array(
		...
		} elseif ( $component == 'cta' ) {
			return zget_option( 'head_show_cta', 'general_options', false, 'no' ) == 'yes' ? true : false;
		} elseif ( $component == 'cart' ) {
			return znfw_is_woocommerce_active() && zget_option( 'woo_show_cart', 'zn_woocommerce_options', false, 1 ) == 1 ? true : false;
		} elseif ( $component == 'logo' ) {
			return zn_get_layout_option( 'head_show_logo', 'general_options', false, 'yes' ) == 'yes' ? true : false;
		} elseif ( $component == 'mainmenu' ) {
			return has_nav_menu( 'main_navigation' ) ? true : false;
		}
	}
}
...
if ( ! function_exists( 'zn_header_display_logo' ) ) {
	...
	*/
	function zn_header_display_logo() {
		...
		if ( zn_get_layout_option( 'head_show_logo', 'general_options', false, 'yes' ) == 'no' ) {
			return; }
	...
	*/
	function zn_header_calltoaction() {
		...
		if ( $head_show_cta == 'yes' ) {
			...
			if ( $calltoaction_style == 'ribbon' || $calltoaction_style == 'lined' ) {
			...
			} elseif ( $calltoaction_style == 'custom' ) {
				...
				if ( isset( $btn_custom ) && ! empty( $btn_custom ) ) :
					foreach ( $btn_custom as $i => $btn ) :
						...
						$classes[] = $btn['cta_hide_xs'] === 'hidden-xs' ? 'hidden-xs' : '';
...
if ( ! function_exists( 'zn_header_main_menu' ) ) {
	...
	*/
	function zn_header_main_menu( $mm_style = 'active-bg' ) {
		...
		if ( ! zn_check_components( 'mainmenu' ) ) {
			return;
		}
...
function zn_header_height( $css ) {
	...
	if ( $zn_head_height_enb != 'default' ) {
		...
		if ( isset( $zn_header_height_scheme[ $zn_header_layout ] ) && ! empty( $zn_header_height_scheme[ $zn_header_layout ] ) ) {
			...
			$final_head_height = (int) $header_height - $hs_others;

...
/** load text domain */
function kl_load_text_domain() {
	load_theme_textdomain( 'zn_framework', THEME_BASE . '/languages' );
}
add_action( 'init', 'kl_load_text_domain' );
...
if ( ! function_exists( 'zn_page_loading' ) ) {
	...
	function zn_page_loading() {
		...
		if ( 'no' != $page_preloader ) {
			...
			} elseif ( 'yes_spinner' == $page_preloader ) {
			...
			} elseif ( 'yes_persp' == $page_preloader ) {
					echo '<div class="preloader-perspective-anim kl-main-bgcolor"></div>';
			} elseif ( 'yes_img_persp' == $page_preloader && ! empty( $page_preloader_img ) ) {
					echo '<div class="preloader-perspective-img"><img src="' . $page_preloader_img . '"></div>';
			} elseif ( 'yes_img_breath' == $page_preloader && ! empty( $page_preloader_img ) ) {
					echo '<div class="preloader-breath-img"><img src="' . $page_preloader_img . '"></div>';
			} elseif ( 'yes_img' == $page_preloader && ! empty( $page_preloader_img ) ) {
...
if ( ! function_exists( 'get_video_from_link' ) ) {
	...
	function get_video_from_link( $string, $css = null, $width = 425, $height = 239, $video_attributes = null ) {
		...
		if ( 'www.youtube.com' == $video_url['host'] || 'youtube.com' == $video_url['host'] || 'www.youtu.be' == $video_url['host'] || 'youtu.be' == $video_url['host'] ) {
			...
			preg_match( '/^.*(?:(?:youtu\.be\/|v\/|vi\/|u\/\w\/|embed\/|shorts\/)|(?:(?:watch)?\?v(?:i)?=|\&v(?:i)?=))([^#\&\?]*).*/', $string, $matches );
			if ( isset( $video_attributes['loop'] ) && ! empty( $video_attributes['loop'] ) ) {
				$extra_options[] = 'playlist=' . $matches[1];
			}

			$string = '<iframe class="' . $css . '" width="' . $width . '" height="' . $height . '" src="//www.youtube.com/embed/' . $matches[1] . '?iv_load_policy=3&amp;enablejsapi=0&amp;wmode=opaque&amp;feature=player_embedded&amp;' . implode( '&amp;', $extra_options ) . '" allow="autoplay; fullscreen"></iframe>';
			...
			$string = preg_replace(
				...
				'<iframe ' . WpkPageHelper::zn_schema_markup( 'video' ) . ' class="youtube-player ' . $css . '" src="//player.vimeo.com/video/$2?' . implode( '&amp;', $extra_options ) . '" width="' . $width . '" height="' . $height . '" allow="autoplay; fullscreen"></iframe>',
				$string
			);
		} else {
	...
	function zn_comment( $comment, $args, $depth ) {
		...
		<div id="comment-<?php comment_ID(); ?>" class="kl-comment__wrapper">
			<div
				...
				<?php printf( __( '<cite class="fn">%s</cite>', 'zn_framework' ), get_comment_author_link() ); ?> <?php echo __( 'says :', 'zn_framework' ); ?>
								<?php
								comment_reply_link(
									array_merge(
										$args,
		...
		if ( function_exists( 'register_sidebar' ) ) {
			...
			register_sidebar(
				array(
					...
					'description'   => esc_html__(
						"This is the default sidebar. You can choose from the theme's options page where
			...
			register_sidebar(
				array(
					...
					'description'   => esc_html__(
						"This is the sidebar for the hidden panel in the header. You can choose from the
								theme's options page where the widgets from this sidebar will be shown.",
						'zn_framework'
					),
...
function adminBarComingSoonNotice( $wp_admin_bar ) {
	...
	if ( is_user_logged_in() && current_user_can( 'administrator' ) && 'yes' == zget_option( 'cs_enable', 'coming_soon_options', false, 'no' ) ) {
		$args = array(
			...
			'meta'  => array(
				'class' => 'znkl_comingsoon_on',
				'title' => __( 'Click to access options and disable it.', 'zn_framework' ),
			),
		);
...
if ( ! function_exists( 'kallyasShowCuteSliderNotice' ) ) {
	...
	function kallyasShowCuteSliderNotice() {
		do_action(
			'wpk_dismissible_notice',
			'error',
			__(
				'The plugin 3D Cute Slider is no longer supported by its author and it was removed from Envato Marketplace.
...
if ( ! function_exists( 'zn_get_target_html' ) ) :
	function zn_get_target_html( $link_target, $target ) {
		...
		} elseif ( 'modal_iframe' == $target ) {
				$link_target = 'data-lightbox="iframe"';
		} elseif ( 'modal_inline' == $target ) {
				$link_target = 'data-lightbox="inline"';
		} elseif ( 'modal_inline_dyn' == $target ) {
				$link_target = 'data-lightbox="inline-dyn"';
		} elseif ( 'smoothscroll' == $target ) {
...
if ( ! function_exists( 'zn_social_share_icons' ) ) {
	function zn_social_share_icons( $args = array() ) {
		$defaults = array(
			...
			'share_media'   => '',
			'mail_body'     => sprintf(
				__( 'You can see it live here %1$s. %2$s Made by %3$s %4$s .', 'zn_framework' ),
		...
		$icons = apply_filters(
			'zn_filter_social_share_icons',
			array(
				'twitter'   => array(
					'url'    => 'https://twitter.com/intent/tweet',
					'icon'   => array(
						'family'  => 'kl-social-icons',
						'unicode' => 'ue82f',
					),
					'params' => array(

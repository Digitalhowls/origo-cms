...
//<editor-fold desc="::: IMPORT FUNCTIONS">
	...
	public static function installDependentPlugins( $demoDirPath )
	{
		...
		// This is useful because we can try to import the content term using their own ids
		self::addDummyTerm();
		...
		if( ! empty($demoName) )
		{
			$demoInfo = Demos::get_demo_info( $demoName );
			if( ! empty($demoInfo) )
			{
				...
				foreach( $demoPlugins as $pluginSlug )
				{
					...
					if( kallyas()->plugins->is_plugin_installed( $pluginSlug ) ){
						if( ! kallyas()->plugins->is_plugin_active( $pluginSlug ) ){
							// Try to activate the plugin
							$result = kallyas()->plugins->activate_plugin( $pluginSlug );
						}
					}
					// Install the plugin and activate it
					else {
						$result = kallyas()->plugins->install_plugin( $pluginSlug );
					}
...
//							error_log( '[DEMO IMPORTER] Error adding the term: ' . $term['name'] . ': ' . $resultInsert->get_error_message() );
	...
	public static function importDemoImages( $demoDirPath )
	{
		...
		$tempData = get_transient( self::TEMP_DATA_TRANS );
		if( ! $tempData || ! isset($tempData['images'])) {
			$tempData['images'] = array();
		}
